    ...
    def update_ai(self, player_rect, all_walls, dest_walls, enemy_bullets,
        ...
        if self._timer >= self.THINK_EVERY:
            ...
            if new_dir is not None:
                ...
                # Przy skręcie w poziom (lewo/prawo) wyrównujemy pozycję pionową Y
                if new_dir[0] != 0:
                    if abs(ty - self.rect.centery) < 12:
                        self.rect.centery = ty
                # Przy skręcie w pion (góra/dół) wyrównujemy pozycję poziomą X
                elif new_dir[1] != 0:
                    if abs(tx - self.rect.centerx) < 12:
        ...
        if hit:
            ...
            if self._ally_dodge_timer == 0:
                adx = self.rect.centerx - ally_hit.rect.centerx
